...
def main():
    ...
    # Nâng lượng bệnh nhân lên hơn 10.000
    patients = generate_patients(10500, phones)
    
    schedule_pattern = generate_doctor_schedules_pattern(doctors)
    # Tăng days lên 45 để với 180 bác sĩ có thể sinh ra > 10.000 lịch trống
    schedules = generate_schedules_from_pattern(schedule_pattern, None, 45)
    
    # Nâng cuộc hẹn và hồ sơ bệnh án lên mốc 10.000
    appointments = generate_appointments(patients, schedules, 10000)
    records = generate_medical_records(appointments, 10000)
    users = generate_user_records(patients, doctors)
    
    # Cập nhật tên các file được xuất ra (Thêm "test")
    save_to_file('doctorstest.txt', doctors)
    save_to_file('patientstest.txt', patients)
    save_to_file('schedulestest.txt', schedules)
    save_to_file('appointmentstest.txt', appointments)
    save_to_file('recordstest.txt', records)
    save_to_file('user_credentialstest.txt', users)
